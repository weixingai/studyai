...
export default function DeepSeekClient({ initialNavItems }: Props) {
  const [content, setContent] = useState<string>('')
  // 获取DeepSeek部分的导航配置
  const deepseekNav = topNavigation.find(nav => nav.key === 'deepseek')
  const navItems = deepseekNav?.sidebar || []
  ...
  const handleNavItemClick = (href: string) => {
    ...
    if (href === '/deepseek') {
      ...
      if (defaultPath) {
        ...
        // 立即加载内容
        const contentPath = defaultPath.replace('/deepseek/', '').replace(/^\/+|\/+$/g, '')
        if (contentPath) {
      ...
      // 立即加载内容
      const contentPath = cleanHref.replace('/deepseek/', '').replace(/^\/+|\/+$/g, '')
      if (contentPath) {
        loadContent(contentPath)
      }
    }
  ...
  const loadContent = async (page: string) => {
    try {
      ...
      const response = await fetch(`/api?action=get-content&page=deepseek/${cleanPage}`);
            ...
            <div className="mt-12 border-t border-gray-200">
              <div className={`grid gap-4 py-6 ${navigation.prev && navigation.next ? 'grid-cols-2' : 'grid-cols-1'}`}>
                ...
                {navigation.prev && (
                  <button
                    onClick={() => {
                      const prevPath = `/deepseek/${navigation.prev!.id}`;
                ...
                {navigation.next && (
                  <button
                    onClick={() => {
                      const nextPath = `/deepseek/${navigation.next!.id}`;
